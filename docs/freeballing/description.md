...
- **Responsive public usernames** — The familiar magnetic letter-card scrubber now supports unique one-to-five-letter names, blank cards for shorter choices, and one-to-five-letter flicks.
...
- **Level editor** — Place, stamp, select, style, animate, score, sonify, and configure boards with centered grid presets, stable formation previews, full undo/redo history, material chips, and versioned foreground motion profiles. Desktop uses compact independently scoped foreground/background inspectors, dedicated Create/Select/Material/Motion workspaces, exact object properties, level music and gutter controls, conventional modifier selection, live selected-motion preview, and distinct Spin Each / Orbit Group controls.
