...
- **Level editor** — Place, stamp, select, style, animate, and configure boards with centered grid presets, stable formation previews, full undo/redo history, material chips, and versioned foreground motion profiles. Desktop uses independently scoped dual-rail inspectors, conventional modifier selection, live selected-motion preview, and distinct Spin Each / Orbit Group controls.
