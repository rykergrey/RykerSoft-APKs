...
**Word Grid:** Tap two tiles to swap them. Swipe adjacent letters horizontally, vertically, or diagonally, changing direction as needed, to lock a word in green. You cannot jump over tiles or use a tile twice in one word. Swaps keep a locked word only while its path remains connected and still spells the word.
...
- Select **Sign in**, choose **Continue with Google**, then create a separate public player name to use Daily Boards, Daily Missions, cloud scores, and community challenges.
