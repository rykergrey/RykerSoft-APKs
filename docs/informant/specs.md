...
- **Current version:** 1.3.0 (versionCode 14)
...
- **Pro unlock:** Google authentication uses the named RykerSoft hub Firebase app (`rykersoft-abe84`); Android uses native Credential Manager through `@capacitor-firebase/authentication`, while Electron/web uses Firebase popup auth
- **Provider keys:** Gemini/Groq/ElevenLabs keys sync only after package entitlement; Firestore reads `providerKeys/com.rykersoft.informant` and denies cross-package access
